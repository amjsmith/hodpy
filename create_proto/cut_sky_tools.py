...
def cut_sky(gal_cat, photsys, hod, Lbox=2000., replication=(0,0,0), zcut=None, mag_cut=None, z0=0.2, with_proto=False):
    """
    ...
    if with_proto:
        position_rep_proto = np.array([gal_cat['proto_x'], gal_cat['proto_y'], gal_cat['proto_z']]).transpose()
    if replication==(0,0,0):
        ...
        for i in range(3):
            ...
            if with_proto:
                position_rep_proto[:,i] += Lbox*replication[i]
    
    gal_cat['x_rep'] = position_rep[:,0]
    gal_cat['y_rep'] = position_rep[:,1]
    gal_cat['z_rep'] = position_rep[:,2]
    if with_proto:
        gal_cat['proto_x_rep'] = position_rep_proto[:,0]
        gal_cat['proto_y_rep'] = position_rep_proto[:,1]
        gal_cat['proto_z_rep'] = position_rep_proto[:,2]
    ...
    if with_proto:
        gal_cat['PROTO_RA'], gal_cat['PROTO_DEC'], gal_cat['PROTO_Z_COSMO'] = cat.pos3d_to_equatorial(position_rep_proto)
...
def make_lightcone(input_file, output_file, hod, photsys, mag_faint, snapshot_redshift=0.2, box_size=2000., observer=(0,0,0), zmax=0.6):
    ...
    cat['proto_x'] -= observer[0]
    cat['proto_y'] -= observer[1]
    cat['proto_z'] -= observer[2]
    
    for dim in 'x','y','z', 'proto_x', 'proto_y', 'proto_z':
    ...
    for i in range(-5,6):
        for j in range(-5,6):
            for k in range(-5,6):
                ...
                cat_new = cut_sky(cat, photsys, hod, Lbox=box_size, replication=(i,j,k), zcut=zmax, mag_cut=mag_faint, z0=snapshot_redshift, with_proto=True)
...
def join_files(galaxy_cutsky, output_file, zmax_low=0.15, zmax=0.6, app_mag_faint=20.2):
    ...
    for file_number in range(1,35):
        print(file_number)
        fff = str(file_number).zfill(2)
        table_i = Table()
        #table_j = Table()
        ...
        for i in range(-5,6):
            for j in range(-5,6):
                for k in range(-5,6):
                    ...
                    try:
                        print((galaxy_cutsky%fff)[:-5]+'_rep%i%i%i.fits'%(i,j,k))
                        table_i_rep = Table.read((galaxy_cutsky%fff)[:-5]+'_rep%i%i%i.fits'%(i,j,k))
    ...
    table_new['PROTO_RA']        = np.array(table['PROTO_RA'],        dtype=np.float32)
    table_new['PROTO_DEC']        = np.array(table['PROTO_DEC'],        dtype=np.float32)
    table_new['Z_COSMO']   = np.array(table['Z_COSMO'],   dtype=np.float32)
    table_new['PROTO_Z_COSMO']   = np.array(table['PROTO_Z_COSMO'],   dtype=np.float32)
    ...
    table_new['proto_x'] = np.array(table['proto_x'],     dtype=np.float32)
    table_new['proto_y'] = np.array(table['proto_y'],     dtype=np.float32)
    table_new['proto_z'] = np.array(table['proto_z'],     dtype=np.float32)
    table_new['proto_x_rep'] = np.array(table['proto_x_rep'], dtype=np.float32)
    table_new['proto_y_rep'] = np.array(table['proto_y_rep'], dtype=np.float32)
    table_new['proto_z_rep'] = np.array(table['proto_z_rep'], dtype=np.float32)
